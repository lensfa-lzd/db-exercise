...
void RmFileHandle::release_page_handle(RmPageHandle& page_handle) {
    ...
    auto free_no = page_handle.page->get_page_id().page_no;
    for (int i = RM_FILE_HDR_PAGE+1; i < file_hdr_.num_pages; i++){
        RmPageHandle ph = fetch_page_handle(i);
        auto current_no = ph.page_hdr->next_free_page_no;
        if (current_no == -1){
            ph.page_hdr->next_free_page_no = free_no;
        }
    }

    if (file_hdr_.first_free_page_no == -1){
        file_hdr_.first_free_page_no = free_no;
    }
}