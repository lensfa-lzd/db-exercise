...
/**
 * @description: 更新页面数据, 如果为脏页则需写入磁盘，
 * 再更新为新页面，更新page元数据(data, is_dirty, page_id)和page table
...
bool BufferPoolManager::delete_page(PageId page_id) {
    ...
    if (page_table_.find(page_id) == page_table_.end()) {
        ...
        if (page->pin_count_ > 0) {
            ...
            page->reset_memory();
            free_list_.emplace_back(static_cast<frame_id_t>(frame_id));
            return true;
        }
    }
}
...
void BufferPoolManager::write_back_page(Page* page) {
    auto pid = page->id_;
